 ...
 */
 /* //Example call for 2D:
 var methods = [];
 generateComparisonLogic(methods, ["up", "down", "left", "right"], 0, ["up", "down", "left", "right"]);
 copy(methods.join("\n\n")); //copy to clipboard
 //paste to text editor and replace '"DIMMENSIONS PLACEHOLDER"' for 'x, y' and 'string dimmensionsPlaceholder' for 'int x, int y'
 
 //Beware that the method index is saved as a global variable so you may have to reset it or refresh the page before running again
 */
...
function getMethodName(elems) {
	...
	return "aetherLogicMethod" + counter;
}
...
function getInnerLogic(elems, indent) {
	...
	for (var i = 0; i < elems.length; i++) {
		...
		for (var j = 0; j < subElements.length; j++) {
			...
			result += "	addTo" + subEl[0].toUpperCase() + subEl.substr(1) + "(newGrid, \"DIMMENSIONS PLACEHOLDER\", share);" + lb;
		}
		result += "}";
	}
	return result;
