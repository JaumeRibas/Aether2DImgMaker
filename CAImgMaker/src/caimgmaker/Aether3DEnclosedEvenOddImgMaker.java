...
import cellularautomata.automata.Aether3DEnclosed;
import cellularautomata.evolvinggrid.EvolvingLongGrid3D;

public class Aether3DEnclosedEvenOddImgMaker {
...
//		args = new String[]{"-1000000", "D:/data/test"};//, "150", "30", "10000"};//debug
		if (args.length == 0) {
			...
			int side = 701;
			if (isRestore) {
				ca = null;//new Aether3DEnclosed(initValOrBackupPath);
			} else {
				ca = new Aether3DEnclosed(initialValue, side).asymmetricSection();
			}
			...
			if (isScanInitialZIndexDefined) {
				imgMaker.createScanningAndCrossSectionEvenOddImages(ca, scanInitialZIndex, 0, colorMapper, colorMapper, 
						ImgMakerConstants.HD_HEIGHT/2, ImgMakerConstants.HD_HEIGHT/2, path + "/img", path + "/backups");
			} else {
				imgMaker.createScanningAndCrossSectionEvenOddImages(ca, 0, colorMapper, colorMapper, 
						ImgMakerConstants.HD_HEIGHT/2, ImgMakerConstants.HD_HEIGHT/2, path + "/img", path + "/backups");
			}	
		}		
	}
