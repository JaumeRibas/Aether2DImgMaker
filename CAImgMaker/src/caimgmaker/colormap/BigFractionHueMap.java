...
public class BigFractionHueMap implements BoundedColorMap<BigFraction> {
	...
	private static final BigFraction hueRange = new BigFraction(220);
	private static final int hueMargin = new BigFraction(255).subtract(hueRange).intValue();
	...
	public Color getColor(BigFraction value) throws IllegalArgumentException {
		...
		float hue = (hueRange.multiply(value.subtract(minValue)).divide(range).bigDecimalValue(7, BigDecimal.ROUND_HALF_UP).floatValue() + hueMargin)/255;
		hue = (hue + (float)1/6)%1;
		hue = 1 - hue;
