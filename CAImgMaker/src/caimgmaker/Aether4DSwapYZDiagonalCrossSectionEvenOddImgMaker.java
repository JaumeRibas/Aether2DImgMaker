...
//						nextArgIndex = 6;
			...
			if (!checkCrossSectionInBounds(ca, zOffsetFromY)) {
				...
				ActionableModel4DYZDiagonalCrossSection<LongModel4D, LongModel3D> crossSection = 
						new ActionableModel4DYZDiagonalCrossSection<LongModel4D, LongModel3D>(ca, zOffsetFromY);
				String imagesPath = path + crossSection.getSubfolderPath();
				int crossSectionZ = 0;
				if (isScanInitialIndexesDefined) {
					imgMaker.createXScanningAndZCrossSectionEvenOddXYImagesFromLongModel3D(
	...
	private static boolean checkCrossSectionInBounds(Model4D grid, int zOffsetFromY) {
		int y = grid.getMinY();
