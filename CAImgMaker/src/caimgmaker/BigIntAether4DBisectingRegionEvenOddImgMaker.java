...
//		args = new String[]{"2000", "D:/data/test"};//debug
		if (args.length == 0) {
			...
			if (isScanInitialZIndexDefined) {
				imgMaker.createXScanningAndZCrossSectionEvenOddImages(bisectingRegion, scanInitialXIndex, 
						0, colorMapper, colorMapper, ImgMakerConstants.HD_WIDTH/2, ImgMakerConstants.HD_HEIGHT/2, imagesPath, backupPath);
			} else {
				imgMaker.createXScanningAndZCrossSectionEvenOddImages(bisectingRegion, 
						0, colorMapper, colorMapper, ImgMakerConstants.HD_WIDTH/2, ImgMakerConstants.HD_HEIGHT/2, imagesPath, backupPath);
			}
		}		
	}
