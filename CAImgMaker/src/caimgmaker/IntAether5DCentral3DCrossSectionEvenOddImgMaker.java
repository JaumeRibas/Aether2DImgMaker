...
public class IntAether5DCentral3DCrossSectionEvenOddImgMaker {
...
//		args = new String[]{"2000", "D:/data/test"};//debug
		if (args.length == 0) {
			...
			if (tmp.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) <= 0 
				...
				SymmetricEvolvingIntGrid5D ca = new IntAether5D(initialValue);
				String path;
				int initialStep = 0;
				if (args.length > 1) {
					path = args[1];
				...
				EvolvingIntGrid3D crossSection = ca.asymmetricSection().crossSectionAtYZ(0, 0);
