...
//		args = new String[]{"-613566757", "0", "D:/data/test"/*, "0", "0", "0"*/};//debug
		if (args.length < 2) {
			System.err.println("You must specify an initial value and a x offset from w for the cross section.");
		} else {
			int initialValue = 0;
...
//			int xScanInitialIndex = 0;
			...
			if (tmp.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) <= 0 
					&& tmp.compareTo(BigInteger.valueOf(Integer.MIN_VALUE)) >= 0) {
				xOffsetFromW = tmp.intValue();
			} else {
				System.err.println("X offset from w out of int range.");
				return;
			}
			if (args.length > 2) {
				path = args[2];
...
//						nextArgIndex = 6;
			...
			while ((ca.getStep() < initialStep || !checkCrossSectionInBounds(ca, xOffsetFromW)) && !finished) {
				finished = !ca.nextStep();
				System.out.println("step: " + ca.getStep());
			}
			if (!checkCrossSectionInBounds(ca, xOffsetFromW)) {
				System.err.println("X offset from w " + xOffsetFromW + " is out of bounds in all steps of Aether 4D with initial value " + initialValue + ".");
			} else {
				...
				if (isScanInitialIndexesDefined) {
					imgMaker.createXScanningAndZCrossSectionEvenOddYZImagesFromIntModel(
	...
	private static boolean checkCrossSectionInBounds(Model4D grid, int xOffsetFromW) {
		...
		while (w <= maxW && (crossSectionX < grid.getMinXAtW(w) || crossSectionX > grid.getMaxXAtW(w))) {
			w++;
			crossSectionX++;
		}
		return w <= maxW;
