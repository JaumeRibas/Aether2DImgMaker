...
public class BigFractionModel3DPattern implements NumericModel3D<BigFraction> {

	protected NumericModel3D<BigFraction> source;
	protected BigFraction min;
	protected BigFraction range;
	
	public BigFractionModel3DPattern(NumericModel3D<BigFraction> grid) throws Exception {
		this.source = grid;
		MinAndMax<BigFraction> minMax = grid.getMinAndMax();
		min = minMax.getMin();
	...
	public BigFraction getFromPosition(int x, int y, int z) throws Exception {
		BigFraction value = source.getFromPosition(x, y, z);
		return value.subtract(min).divide(range);
	}
