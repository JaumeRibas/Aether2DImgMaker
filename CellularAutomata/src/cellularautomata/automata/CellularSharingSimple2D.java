...
/**
 * A simplified implementation of the Cellular Sharing automaton for review and testing purposes
 * 
 * @author Jaume
 *
 */
public class CellularSharingSimple2D extends LongCellularAutomaton2D {	
	 ...
	 */
	public CellularSharingSimple2D(long initialValue) {
	...
	public boolean nextStep(){
		...
		for (int x = 0; x < this.grid.length; x++) {
			for (int y = 0; y < this.grid[0].length; y++) {
				//Share the positon's value with its neighbors (von Neumann) using the Sharing algorithm
				...
				if (value > 0) {
					...
					ArrayList<LongNeighbor> neighbors = new ArrayList<LongNeighbor>(4);						
					long neighborValue;
					if (x < grid.length - 1)
