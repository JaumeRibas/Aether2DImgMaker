...
public class SimpleLongAether5DTopplingAlternationCompliance implements SymmetricBooleanModel5D, IsotropicHypercubicModel5DA {	
	 ...
	 */
	public SimpleLongAether5DTopplingAlternationCompliance(long initialValue) {
	...
	public Boolean nextStep() {
		...
		if (boundsReached) {
			...
			newGrid = new long[newSide][newSide][newSide][newSide][newSide];
			topplingAlternationCompliance = new HypercubicBooleanArray(5, newSide);
			...
			topplingAlternationCompliance = new HypercubicBooleanArray(5, grid.length);
		}
			...
			for (int j = 0; j < grid.length; j++) {
				for (int k = 0; k < grid.length; k++) {
					for (int l = 0; l < grid.length; l++) {
						for (int m = 0; m < grid.length; m++, isItCurrentPositionsTurnToTopple = !isItCurrentPositionsTurnToTopple) {
							...
							topplingAlternationCompliance.set(new Coordinates(i + indexOffset, j + indexOffset, k + indexOffset, l + indexOffset, m + indexOffset), toppled == isItCurrentPositionsTurnToTopple);
						}
					}
				}
			}
