...
import cellularautomata.arrays.HypercubicBigIntArray;
import cellularautomata.arrays.HypercubicNumericArray;
 ...
 *
 */
public class SimpleBigIntAether implements IsotropicHypercubicNumericModelAsymmetricSection<BigInt> {//TODO fix 1D error
	...
	@Override
	public BigInt getFromPosition(Coordinates coordinates) {
		int[] indexes = new int[gridDimension];
	...
	public Boolean nextStep() {
		...
		HypercubicBigIntArray newGrid = null;
		...
		if (boundsReached) {
			boundsReached = false;
			newSide = grid.getSide() + 2;
			indexOffset = 1;
		} else {
			newSide = grid.getSide();
		}
		newGrid = new HypercubicBigIntArray(gridDimension, newSide);
		newGrid.fill(BigInt.ZERO);
	...
	class AetherConsumer implements Consumer<Coordinates> {
		...
		private HypercubicNumericArray<BigInt> newGrid;
		private int indexOffset;
		
		public AetherConsumer(HypercubicNumericArray<BigInt> newGrid, int indexOffset) {
		...
		public void accept(Coordinates currentIndexes) {
			...
			BigInt value = grid.get(currentIndexes);
			int relevantNeighborCount = 0;
			BigInt[] relevantNeighborValues = new BigInt[neighborCount];
			...
			for (int axis = 0; axis < gridDimension; axis++) {
				...
				if (indexOnAxis < gridSide - 1) {
					...
					BigInt neighborValue = grid.get(new Coordinates(indexes));
					...
					indexes[axis] = indexOnAxis + 1;
					BigInt neighborValue = BigInt.ZERO; 
					if (neighborValue.compareTo(value) < 0) {
						relevantNeighborValues[relevantNeighborCount] = neighborValue;
						int[] neighborIndexes = indexes.clone();
						Utils.addToArray(neighborIndexes, indexOffset);
			...
			if (relevantNeighborCount > 0) {
				...
				BigInt previousNeighborValue = BigInt.ZERO;
				...
				for (int neighborIndex = 0; neighborIndex < relevantNeighborCount; neighborIndex++, shareCount--, isFirst = false) {
					BigInt neighborValue = relevantNeighborValues[neighborIndex];
	...
	@Override
	public void forEach(Consumer<? super BigInt> consumer) {
