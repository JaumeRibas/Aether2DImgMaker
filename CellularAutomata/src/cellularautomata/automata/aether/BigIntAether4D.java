...
public class BigIntAether4D implements SymmetricEvolvingNumberGrid4D<BigInt>, IsotropicGrid4DA, Serializable {
	...
	private static boolean topplePositionSortedNeighbors(BigInt[][][][] newWSlices, BigInt value, int x, int y, int z, 
		...
		if (!share.equals(BigInt.ZERO)) {
			...
			for (int j = 0; j < neighborCount; j++) {
				int[] nc = neighborCoords[j];int nW = nc[0], nX = nc[1], nY = nc[2], nZ = nc[3];
				newWSlices[nW][nX][nY][nZ] = newWSlices[nW][nX][nY][nZ].add(share);
			}
		}
	...
	private static boolean topplePositionSortedNeighbors(BigInt[][][][] newWSlices, BigInt value, int x, int y, int z, BigInt[] asymmetricNeighborValues,
		...
		if (!share.equals(BigInt.ZERO)) {
			...
			for (int j = 0; j < asymmetricNeighborCount; j++) {
				int[] nc = asymmetricNeighborCoords[j];int nW = nc[0], nX = nc[1], nY = nc[2], nZ = nc[3];
				newWSlices[nW][nX][nY][nZ] = newWSlices[nW][nX][nY][nZ].add(share.multiply(BigInt.valueOf(asymmetricNeighborShareMultipliers[j])));
			}
		}
		BigInt previousNeighborValue = neighborValue;
		shareCount -= asymmetricNeighborSymmetryCounts[0];
	...
	private static boolean topplePositionSortedNeighbors(BigInt[][][][] newWSlices, BigInt value, int x, int y, int z, BigInt[] neighborValues,
		...
		if (!share.equals(BigInt.ZERO)) {
			...
			for (int j = 0; j < neighborCount; j++) {
				int[] nc = neighborCoords[j];int nW = nc[0], nX = nc[1], nY = nc[2], nZ = nc[3];
				newWSlices[nW][nX][nY][nZ] = newWSlices[nW][nX][nY][nZ].add(share.multiply(BigInt.valueOf(neighborShareMultipliers[j])));
			}
		}
		BigInt previousNeighborValue = neighborValue;
		shareCount--;
