...
public class ShortAether4DMT extends SymmetricShortCellularAutomaton4D {
	 ...
	 */
	public ShortAether4DMT(short initialValue, short backgroundValue, int threadCount) {
		if (backgroundValue > initialValue) {
			...
			if (maxValue.compareTo(BigInteger.valueOf(Short.MAX_VALUE)) > 0) {
				throw new IllegalArgumentException("Resulting max value " + maxValue 
						+ " exceeds implementation's limit (" + Short.MAX_VALUE 
	 ...
	 * 
	 * @param data an instance of {@link CustomSymmetricShortCA4DData}
	 * @param threadCount the number of threads to uses
	 */
	public ShortAether4DMT(CustomSymmetricShortCA4DData data, int threadCount) {
		this.threadCount = threadCount;
	...
	public boolean nextStep() throws InterruptedException, ExecutionException{
		...
		if (split) {
			for (int i = 0; i < threadCount; i++) {
				newGrid[sectionBounds[i][0]] = buildGridBlock(sectionBounds[i][0], (short) 0);
				newGrid[sectionBounds[i][1]] = buildGridBlock(sectionBounds[i][1], (short) 0);
				calls.add(new Aether4DComputer(grid, newGrid, sectionBounds[i][0], sectionBounds[i][1]));
			}
		} else {
			newGrid[0] = buildGridBlock(0, (short) 0);
			if (grid.length > 1) {
				newGrid[grid.length - 1] = buildGridBlock(grid.length - 1, (short) 0);
			}
	...
	private short[][][] buildGridBlock(int w, short value) {
		short[][][] newGridBlock = new short[w + 1][][];
		for (int x = 0; x < newGridBlock.length; x++) {
			newGridBlock[x] = new short[x + 1][];
			for (int y = 0; y < newGridBlock[x].length; y++) {
				newGridBlock[x][y] = new short[y + 1];
				if (value != 0) {
	...
	public short getNonSymmetricValueAt(int w, int x, int y, int z){	
		if (w < grid.length 
				&& x < grid[w].length 
				&& y < grid[w][x].length 
				&& z < grid[w][x][y].length) {
	...
	class Aether4DComputer implements Callable<ComputingResult> {
		...
		public Aether4DComputer(short[][][][] grid2, short[][][][] newGrid2, int start, int finish) {
			this.grid = grid2;
			this.newGrid = newGrid2;
			this.start = start;
			this.finish = finish;
			maxWMinusOne = newGrid2.length - 2;
		}
							...
							if (relevantNeighborCount > 0) {
								...
								for (int i = 0; i < relevantNeighborCount; i++,isFirst = false) {
									...
									if (neighborValue != previousNeighborValue || isFirst) {
										short shareCount = (short) (relevantNeighborCount - i + 1);
										short toShare = (short) (value - neighborValue);
		...
		public ComputingResult call() {
			if (finish - start + 1 > 1) {
				...
				int nextW = w + 1;
				if (nextW < finish) {
					newGrid[nextW] = buildGridBlock(nextW, (short) 0);
				}
	...
	@Override
	public CustomSymmetricShortCA4DData getData() {
		return new CustomSymmetricShortCA4DData(grid, initialValue, backgroundValue, currentStep, boundsReached, maxX, maxY, maxZ);
	}
