...
public class PolarAether2D extends SymmetricLongCellularAutomaton2D {	
	 ...
	 */
	public PolarAether2D(long initialValue, long backgroundValue) {
		...
		for (int x = 0, nextX = 1; x < grid.length; x++, nextX++, isFirstBlock = false) {
			...
			for (int y = 0; y <= x; y++) {
				...
				long right = getValueAt(x + 1, y),
						left = getValueAt(x - 1, y),
						up = getValueAt(x, y + 1),
						down = getValueAt(x, y - 1);
				if (value >= 0) {
					value = positiveAether(newGrid, x, y, value, right, left, up, down);
				} else if (value < 0) {
					value = negativeAether(newGrid, x, y, value, right, left, up, down);
				}
				newGrid[x][y] += value;
			}
			if (!isFirstBlock) {
	...
	private long positiveAether(long[][] newGrid, int x, int y, long value, long right, long left, long up, long down) {
		long[] neighborValues = new long[4];
		byte[] neighborDirections = new byte[4];
		int relevantNeighborCount = 0;
		if (right < value) {
			neighborValues[relevantNeighborCount] = right;
			neighborDirections[relevantNeighborCount] = RIGHT;
			relevantNeighborCount++;
		}
		if (left < value) {
			neighborValues[relevantNeighborCount] = left;
			neighborDirections[relevantNeighborCount] = LEFT;
			relevantNeighborCount++;
		}
		if (up < value) {
			neighborValues[relevantNeighborCount] = up;
			neighborDirections[relevantNeighborCount] = UP;
			relevantNeighborCount++;
		}
		if (down < value) {
			neighborValues[relevantNeighborCount] = down;
		...
		if (relevantNeighborCount > 0) {
			...
			value = aether(newGrid, x, y, value, neighborValues, neighborDirections, relevantNeighborCount);	
		}
		return value;
	}
	
	private long negativeAether(long[][] newGrid, int x, int y, long value, long right, long left, long up, long down) {
		long[] neighborValues = new long[4];
		byte[] neighborDirections = new byte[4];
		int relevantNeighborCount = 0;
		if (right > value) {
	...
	private long aether(long[][] newGrid, int x, int y, long value, 
			long[] neighborValues, byte[] neighborDirections, int relevantNeighborCount) {
