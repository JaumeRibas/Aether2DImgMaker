...
public class SizeLimitedAnisotropicIntGrid5DBlock implements IntGrid5D, AnisotropicGrid5DA, Serializable {
	...
	public int maxV;
	public int minV;
	private AnisotropicIntGrid5DSlice[] slices;

	public SizeLimitedAnisotropicIntGrid5DBlock(int minV, long maxBytes) {
		this.minV = minV;
		...
		slices = new AnisotropicIntGrid5DSlice[vLength];
		maxV = minV + vLength - 1;
		for (int v = minV, i = 0; v <= maxV; v++, i++) {
			slices[i] = new AnisotropicIntGrid5DSlice(v);
		}
	}
	...
	public void setSlice(int v, AnisotropicIntGrid5DSlice slice) {
		slices[v - minV] = slice;
	}
	
	public AnisotropicIntGrid5DSlice getSlice(int v) {
	...
	private static int getMaxVLength(int minV, long maxBytes) {
		...
		while (roundedSize <= maxBytes) {
			size += AnisotropicIntGrid5DSlice.getSliceSize(v) + Integer.BYTES;
