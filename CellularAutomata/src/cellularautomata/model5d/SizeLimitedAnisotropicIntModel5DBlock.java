...
public class SizeLimitedAnisotropicIntModel5DBlock implements IntModel5D, AnisotropicModel5DA, Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2001779439888705324L;

	public static final int MIN_LENGTH = 2;

	public int maxV;
	public int minV;
	private AnisotropicIntModel5DSlice[] slices;

	public SizeLimitedAnisotropicIntModel5DBlock(int minV, long maxBytes) {
		this.minV = minV;
		...
		slices = new AnisotropicIntModel5DSlice[vLength];
		maxV = minV + vLength - 1;
		for (int v = minV, i = 0; v <= maxV; v++, i++) {
			slices[i] = new AnisotropicIntModel5DSlice(v);
		}
	}

	public void setValueAtPosition(int v, int w, int x, int y, int z, int initialValue) {
	...
	public void setSlice(int v, AnisotropicIntModel5DSlice slice) {
		slices[v - minV] = slice;
	}
	
	public AnisotropicIntModel5DSlice getSlice(int v) {
	...
	private static int getMaxVLength(int minV, long maxBytes) {
		...
		while (roundedSize <= maxBytes) {
			size += AnisotropicIntModel5DSlice.getSliceSize(v) + Integer.BYTES;
			roundedSize = Utils.roundUpToEightMultiple(size);
			v++;
			vLength++;
		}
		return vLength - 2; //subtract one more to leave room for the extra slice being computed
	}
