...
public class SizeLimitedAnisotropicLongModel5DBlock implements LongModel5D, AnisotropicModel5DA, Serializable {
	...
	public int maxV;
	public int minV;
	private AnisotropicLongModel5DSlice[] slices;

	public SizeLimitedAnisotropicLongModel5DBlock(int minV, long maxBytes) {
		this.minV = minV;
		...
		slices = new AnisotropicLongModel5DSlice[vLength];
		maxV = minV + vLength - 1;
		for (int v = minV, i = 0; v <= maxV; v++, i++) {
			slices[i] = new AnisotropicLongModel5DSlice(v);
		}
	}
	...
	public void setSlice(int v, AnisotropicLongModel5DSlice slice) {
		slices[v - minV] = slice;
	}
	
	public AnisotropicLongModel5DSlice getSlice(int v) {
	...
	private static int getMaxVLength(int minV, long maxBytes) {
		...
		while (roundedSize <= maxBytes) {
			size += AnisotropicLongModel5DSlice.getSliceSize(v) + Integer.BYTES;
