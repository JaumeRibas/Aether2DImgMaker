...
public class AnisotropicIntModel5DSlice implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -327535155179700288L;

	private static final int PRIMITIVE_SIZE = Integer.BYTES;

	private int[][][][] data;
	
	public AnisotropicIntModel5DSlice(int v) {
		data = Utils.buildAnisotropic4DIntArray(v + 1);
	}

	public void setAtPosition(int v, int x, int y, int z, int value) {
		data[v][x][y][z] = value;	
	}
	
	public int getFromPosition(int v, int x, int y, int z) {
		return data[v][x][y][z];
	}
	
	public void addToPosition(int v, int x, int y, int z, int value) {
		data[v][x][y][z] += value;
	}
