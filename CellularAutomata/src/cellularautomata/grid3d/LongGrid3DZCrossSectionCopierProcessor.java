...
public class LongGrid3DZCrossSectionCopierProcessor implements GridProcessor<LongGrid3D> {
	...
	public LongGrid2D getCopy(int crossSectionZ) {
			...
			if (copyZ >= gridBlock.getMinZ() && copyZ <= gridBlock.getMaxZ()) {
				...
				for (int x = gridBlock.getMinXAtZ(copyZ); x <= maxX; x++) {
					...
					for (int y = localMinY, i = 0; y <= localMaxY; y++, i++) {
						slice[i] = gridBlock.getValueAtPosition(x, y, copyZ);
					}
