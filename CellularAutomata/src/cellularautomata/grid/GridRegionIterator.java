...
public abstract class GridRegionIterator<G extends GridRegion, T> implements Iterator<T> {
	...
	public GridRegionIterator(G region) {
		this.region = region;
	...
	public T next() {
		...
		T next = null;
		try {
			next = getFromGridPosition(immutableCoords);
		} catch (Exception e) {
	...
	protected abstract T getFromGridPosition(Coordinates coordinates) throws Exception;
