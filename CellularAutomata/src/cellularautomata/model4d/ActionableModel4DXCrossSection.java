...
public class ActionableModel4DXCrossSection<G1 extends Model4D, G2 extends Model3D> 
	extends ActionableModel3D<G2> {

	protected int x;	
	protected ActionableModel4D<G1> source;

	public ActionableModel4DXCrossSection(ActionableModel4D<G1> grid, int x) {
	...
	private class InternalProcessor implements ModelProcessor<G1> {
		...
		public void processModelBlock(G1 gridBlock) throws Exception {
			if (x >= gridBlock.getMinX() && x <= gridBlock.getMaxX()) {
				triggerProcessModelBlock((G2) gridBlock.crossSectionAtX(x));
			}
		}
