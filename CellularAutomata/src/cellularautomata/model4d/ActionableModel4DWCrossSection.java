...
public class ActionableModel4DWCrossSection<G1 extends Model4D, G2 extends Model3D> 
	extends ActionableModel3D<G2> {

	protected int w;	
	protected ActionableModel4D<G1> source;

	public ActionableModel4DWCrossSection(ActionableModel4D<G1> grid, int w) {
	...
	private class InternalProcessor implements ModelProcessor<G1> {
		...
		public void processModelBlock(G1 gridBlock) throws Exception {
			if (w >= gridBlock.getMinW() && w <= gridBlock.getMaxW()) {
				triggerProcessModelBlock((G2) gridBlock.crossSectionAtW(w));
			}
		}
