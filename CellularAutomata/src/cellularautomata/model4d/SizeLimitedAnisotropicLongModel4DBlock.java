...
public class SizeLimitedAnisotropicLongModel4DBlock implements LongModel4D, AnisotropicModel4DA, Serializable {
	...
	public int maxW;
	public int minW;
	private AnisotropicLongModel4DSlice[] slices;

	public SizeLimitedAnisotropicLongModel4DBlock(int minW, long maxBytes) {
		this.minW = minW;
		...
		slices = new AnisotropicLongModel4DSlice[wLength];
		maxW = minW + wLength - 1;
		for (int w = minW, i = 0; w <= maxW; w++, i++) {
			slices[i] = new AnisotropicLongModel4DSlice(w);
		}
	}
	...
	public void setSlice(int w, AnisotropicLongModel4DSlice slice) {
		slices[w - minW] = slice;
	}
	
	public AnisotropicLongModel4DSlice getSlice(int w) {
	...
	private static int getMaxWLength(int minW, long maxBytes) {
		...
		while (roundedSize <= maxBytes) {
			size += AnisotropicLongModel4DSlice.getSliceSize(w) + Integer.BYTES;
